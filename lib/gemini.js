...
// Keep this comfortably under the route's 120s maxDuration so our own
// "taking longer than expected" error can fire before Vercel's hard
// platform cutoff kills the function with a generic error instead.
...
export async function generateReport(repoInfo, curatedFiles) {
  ...
  const response = await withTimeout(
    ...
    "Gemini did not respond within the allotted time."
  );
